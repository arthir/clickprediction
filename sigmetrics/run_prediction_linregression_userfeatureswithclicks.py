...
def linear_regression(cols, feature_name, fout2):
    ...
    for i in y_train_temp:
        if "LogClicks" in file_prefix:
            y_train.append(np.log(float(i[1])+1))
        else:
            y_train.append(float(i[1]))
    for i in y_test_temp:
        if "LogClicks" in file_prefix:
            y_test.append(np.log(float(i[1])+1))
        else:
    ...
    for i, pred in enumerate(y_pred):
        fout2.write(feature_name + "\t" + str(y_test[i]) + "\t" + str(y_pred[i]) + "\n")
...
for click_feature_name, click_feature_cols in share_features.items():
    print click_feature_name
    linear_regression(click_feature_cols, click_feature_name, fout2)
    for feature_name, feature_cols in features.items():
        ...
        linear_regression(combo_feature_cols, combo_feature_name, fout2)

for feature_name, feature_cols in features.items():
    linear_regression(feature_cols,feature_name, fout2)
