...
def multi_classification(cols, feature_name):
    ...
    for est in clf.estimators_:
        print est.support_vectors_.shape
        print "support: ",  est.n_support_
        print est.coef_
    #print(sklearn.metrics.classification_report(y_test, y_pred))
    if weighted_by_volume == 'True':        
        sample_weight = y_test_temp[:,1] # weighted by # clicks
        prf1s = sklearn.metrics.precision_recall_fscore_support(y_test, y_pred, average=None, labels=class_ranges, sample_weight=sample_weight)
        acc = sklearn.metrics.accuracy_score(y_test, y_pred, sample_weight=sample_weight)
    else:
